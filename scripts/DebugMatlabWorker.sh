...
start_matlab() {
    ...
    # Run the MATLAB command in a while loop
    while true; do
        matlab -nodesktop -nosplash -noFigureWindows -prefersoftwareopengl -r "$mat_command"
        echo "MATLAB exited with status $?. Restarting..."

        error_file="$start_directory/worker_$worker_index.error"
        echo "Checking for error file: $error_file"

        # Sleep for a minute. Check if there is an error file for the worker.
        # If there is, sleep for another minute and check again. When the error
        # file is removed, break out of the loop and restart MATLAB.
        while true; do
            sleep 1
