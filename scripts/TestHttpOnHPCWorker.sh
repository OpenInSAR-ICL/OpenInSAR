...
load_modules() {
    # If we're IDX 1 or 2, load python. 
    # Indexing starts at 1.
    # Note index might be a string, so use -eq
    if [ "$worker_index" -gt 2 ]; 
    then
        ...
        {
            module load tools/prod 2>&1
            module load Python/3.10.8-GCCcore-12.2.0 2>&1
        } || {
...
start_matlab() {
    ...
    # If we're IDX 1, run python server.
    # Indexing starts at 1.
    # Note index might be a string, so use -eq
    if [ "$worker_index" -eq 1 ]; 
    then
        python3 TestHttpServer.py
    fi

    # If we're IDX 2, run python client.
    if [ "$worker_index" -eq 2 ]; 
    then
        python3 TestHttpClient.py
    fi

    # Any other IDX, run matlab.
    if [ "$worker_index" -gt 2 ]; 
    then
